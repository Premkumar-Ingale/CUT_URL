...
const DashboardPage = () => {
  ...
  useEffect(() => {
    if (error) {
      fetch(`${(import.meta.env.VITE_API_URL || 'https://cut-url-crlg.onrender.com').replace(/\/$/, '')}/api/create/no`, {credentials: 'include'})
      .then(res => res.json())
        ...
        {isLoading ? (
          ...
          urls.map(url => (
            <UserUrl
              ...
              shortUrl={`${(import.meta.env.VITE_API_URL || 'https://cut-url-crlg.onrender.com').replace(/\/$/, '')}/${url.short_url}`}
              clicks={url.clicks}
